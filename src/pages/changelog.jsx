            ...
            {items.map((item, i) => (
              <div key={i} className="mb-3">
                <Disclosure>
                  {({ open }) => (
                    <>
                      <Disclosure.Button className="bg-slate-50 dark:bg-slate-700 dark:bg-opacity-60 rounded-t-md flex justify-between cursor-pointer transition duration-150 font-medium w-full text-start text-base text-kog-white dark:text-slate-300 px-8 py-4">
                        <span>
                          {item.version}
                      ...
                      <Disclosure.Panel>
                        <div className="text-sm text-kog-white font-normal bg-kog-999 dark:bg-slate-900 dark:text-slate-300 rounded-b-md dark:border dark:border-slate-700 dark:border-t-0 border border-slate-100 border-t-0">
                          <div className="px-8 py-4">
                            {item.changes.map((data, j) => (
                              <div key={j}>
                                <div className="flex space-x-3 items-center mt-2 text-kog-white dark:text-slate-300 text-sm">
      ...
      <div className="lg:col-span-4 col-span-12">
        <Card title="CHANGELOG">
          ...
          <ul className="space-y-3 mt-6 divide-y dark:divide-slate-700 divide-slate-100">
            {items.map((item, i) => (
              <li
                className="flex justify-between items-center text-xs text-kog-white dark:text-slate-300 pt-3"
                key={i}
              >
