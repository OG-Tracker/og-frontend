...
const GovDetailsPage = () => {
  const { id } = useParams(); // Retrieve 'id' from the URL.
  const [isPopupOpen, setPopupOpen] = useState(false);

  // Find the matching motion in the Redux state.
  const project = useSelector((state) =>
    state.gov1?.gov1?.find((proj) => String(proj.motion) === id)
  );


  if (!project) {
    // Render an error message if no project matches the motion.
    return <div className="text-center text-lg text-red-500">Motion not found</div>;
  }
  ...
  const {
    ...
    articles,
    pow,
    ...tasks // Destructure all remaining fields for tasks
  } = project;

  // Filter non-null tasks
  const taskList = Object.entries(tasks)
    .filter(([key, value]) => key.startsWith("task") && value) // Select only task fields with non-null values
    .map(([key, value]) => value);

  return (
    <div className="w-full p-6 space-y-6 mb-10">
      <div className="border border-gray-700 bg-gradient-to-t from-kog-50 to-gray-800 shadow-[0px_0px_20px_10px_rgba(0,0,0,0.4)] rounded-lg ">
        <div className="bg-seven p-6 rounded bg-cover font-special min-h-[80vh]">
          <h2 className="text-2xl font-bold text-white mb-6 border-b border-gray-600 pb-4 flex items-center justify-between">
            <span className="text-gray-500 text-5xl font-special  w-full max-w-[1px]">#{motion}</span>
            ...
            {pow && !pow.startsWith('-') ? (
              !pow.includes('0') ? (
                <a
                  href={pow}
                  target="_blank"
                  className="px-4 py-2 bg-gradient-to-t from-green-900 to-gray-700 text-white rounded border border-gray-500 font-bold hover:from-green-900 hover:to-gray-800 animate-glowgray cursor-pointer"
                >
                  Proof Of Work
                </a>
              ) : (
                <div
                  className="px-4 py-2 bg-gradient-to-t from-green-900 to-gray-800 text-white rounded border border-gray-500 font-bold cursor-not-allowed"
                />
              )
            ) : (
              <Tooltip
                title="d"
                placement="bottom"
                className="btn btn-outline-kog-900 bg-kog-100 dark:text-kog-900 dark:hover:bg-kog-900 dark:hover:text-white rounded-full px-4 py-1 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.3)]"
                arrow
                allowHTML
                interactive
                theme="custom-gray"
                maxWidth="220px"
                content={<div className="dark:text-slate-300 text-kog-white text-sm">N/A</div>}
              >
                <div className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0 cursor-not-allowed">
                  <div className="px-4 py-2 bg-gradient-to-t from-gray-700 to-gray-900 text-white rounded border cursor-not-allowed border-gray-500 font-bold hover:from-gray-800 hover:to-gray-800 animate-glowgray">
                    Proof Of Work
                  </div>
                </div>
              </Tooltip>
            ...
            {/* <a href={pow} target="_blank" className="px-4 py-2 bg-gray-700 text-white rounded border border-gray-500 font-bold hover:bg-gray-800 animate-glowgray cursor-pointer">Proof Of Work</a> */}


            {proplink && !proplink.startsWith('-') ? (

              <Link target="_blank" to={proplink} className=""><Icon className="text-4xl text-gray-500 hidden sm:inline-block hover:text-white font-bold" icon="heroicons:book-open" /></Link>
            ) : (
              <Tooltip title="d" placement="bottom" className="btn btn-outline-kog-900 bg-kog-100 dark:text-kog-900 hidden sm:inline-block dark:hover:bg-kog-900 dark:hover:text-white rounded-full px-4 py-1 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.3)]" arrow allowHTML interactive theme="custom-gray" maxWidth="220px" content={<div className="dark:text-slate-300 text-kog-white text-sm">N/A</div>}>
                <div className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0 cursor-not-allowed">

                  <Icon className="text-xl text-gray-500 font-bold " icon="heroicons:book-open" />
          ...
          <div className=" gap-6 ">
            ...
            <div className="">
              <h4 className="text-center text-white font-bold mt-10 text-lg sm:text-3xl">{ptitle}</h4>
              <div className="mt-6 grid grid-cols-4 max-w-4xl mx-auto lg:grid-cols-8 w-full shadow-[inset_0px_0px_25px_rgba(0,0,0)] justify-center items-center sm:divide-x divide-gray-700 border rounded-lg sm:rounded-md border-gray-700 py-2  bg-transparent">
                {twitter && !twitter.startsWith('-') ? (
                  <a href={twitter} target="_blank" className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
                      <Icon className={'m-auto text-white'} icon="pajamas:twitter" />
                    </div>
                  </a>
                ) : (
                  <Tooltip title="d" placement="bottom" className="  rounded-full px-4 py-1 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.3)] bg-kog-900" arrow allowHTML interactive theme="custom-gray" maxWidth="220px" content={<div className=" text-white text-sm">N/A</div>}>
                    <div className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0 cursor-not-allowed">
                      <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] bg-gradient-to-t from-kog-300 to-gray-700 flex justify-center items-center m-auto min-w-[45px] min-h-[45px]">
                        <Icon className={'m-auto text-white'} icon="pajamas:twitter" />
                      </div>
                    </div>
                  </Tooltip>
                )}
                {website && !website.startsWith('-') ? (
                  <a href={website} target="_blank" className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
                ...
                {github && !github.startsWith('-') ? (
                  <a href={github} target="_blank" className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
                ...
                {youtube && !youtube.startsWith('-') ? (
                  <a href={youtube} target="_blank" className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
                ...
                {articles && !articles.startsWith('-') ? (
                  <a href={articles} target="_blank" className="flex justify-center items-center text-2xl px-2 my-[5px] sm:my-0">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
                ...
                {sublink && !sublink.startsWith('-') ? (
                  <a href={sublink} target="_blank" className="flex justify-center items-center px-2">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
                ...
                {palink && !palink.startsWith('-') ? (
                  <a href={palink} target="_blank" className="flex justify-center items-center px-2">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
                ...
                {scanlink && !scanlink.startsWith('-') ? (
                  <a href={scanlink} target="_blank" className="flex justify-center items-center px-2">
                    <div className="rounded-md p-2 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] transition-all duration-200 ease-in-out bg-gradient-to-t hover:bg-gradient-to-l from-kog-300 to-gray-700 hover:bg-kog-500 flex justify-center items-center m-auto min-w-[45px] min-h-[45px] hover:min-w-full hover:shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)]">
              ...
              <div class="sm:divide-x divide-y grid sm:grid-cols-3 divide-solid divide-gray-700 text-lg max-w-6xl mx-auto text-center border border-gray-600 rounded shadow-[inset_0px_0px_15px_5px_rgba(0,0,0,0.5)] mt-5">
                <div class="py-4">
                  <div className="text-white font-semibold">
                    <span className="text-gray-400 mb-2 inline-block">REQUESTED</span> <br />  <span className=" text-xl">{reqdot}</span>
                  </div>

                </div>

                <div class="py-4 font-semibold"><span className="text-gray-400 mb-2 inline-block">PROPOSER</span> <br /> <span className=" text-xl">{proposername}</span> </div>

                <div className="py-4 text-white font-semibold items-center w-full ">
                  <span className="text-gray-400 mb-2 inline-block ">BENEFICIARY</span>
                    ...
                    <div className="">
                      ...
                      >
                        ...
                        <span className="truncate block">
                          {benadd ? `${benadd.slice(0, 4)}...${benadd.slice(-3)}` : benadd}
                        </span>
                      </Link>
                    </div>
                  ) : (
            ...
            <div className="grid sm:grid-cols-2 space-x-6 items-stretch mt-4 relative sm:max-h-[500px]">
              {/* SUMMARY */}
              <div className="text-center mt-7 sm:flex sm:flex-col h-full">
                <h5 className="text-2xl font-semibold text-gray-400">SUMMARY</h5>
                <div className="mt-2 sm:flex sm:justify-center sm:items-center rounded-md shadow-[inset_0px_0px_15px_2px_rgba(0,0,0,1)] p-6 border border-gray-700 h-full max-h-[300px] overflow-y-auto">
                  <p className="text-white text-lg">{summary}</p>
                </div>
              </div>

              {/* TASKS */}
              <div className="text-center mt-7 flex flex-col h-full">
                <h5 className="text-2xl font-semibold text-gray-400">TASKS</h5>
                <div className="flex justify-center items-center shadow-[inset_0px_0px_15px_2px_rgba(0,0,0,1)] rounded-md p-6 w-full mt-2 border border-gray-700 h-full max-h-[300px] overflow-y-auto">
                  {taskList.length > 0 ? (
                    <ul
                      className="text-white space-y-2 w-full py-6"
                      style={{ maxHeight: "300px" }} // Set a max height
                    >
                      {taskList.map((task, index) => (
                        <li
                          className="border text-lg text-white rounded p-2 px-3 border-gray-800 shadow-[0px_0px_15px_2px_rgba(0,0,0,1)] bg-gradient-to-l from-gray-800 to-transparent w-full"
                          key={index}
                        >
                          {task}
                        </li>
                    ...
                    <div className="text-white text-xl flex items-center justify-center h-full w-full">
