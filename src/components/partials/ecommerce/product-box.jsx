...
const ProductBox = ({ item, wish }) => {
  ...
  const number = item.reqDot.replace(/[^\d.-]/g, ''); // Extracts the number, including decimals and negative sign if any
  ...
  return (
    ...
    <Card bodyClass=" rounded-md" className="group shadow-[inset_0px_0px_10px_7px_rgba(0,0,0,0.4)] hover:shadow-[0px_0px_7px_7px_rgba(166,0,157,0.3)]  border-kog-700 border-2 hover:bg-kog-100 hover:bg-opacity-70  duration-150 ease-out hover:ease-in">
      ...
      <div className="   p-4 min-h-[370px] ">
        <Link to={item.id}>
          <div className="flex justify-between items-center border-b border-kog-300 pb-3 ">
            ...
            <span className="text-2xl	border-kog-300 text-kog-white px-2 py-2 rounded-full mx-1 p-1 shadow-[0px_0px_10px_5px_rgba(0,0,0,0.4)] bg-gradient-to-t from-kog-300 to-kog-800   ml-3">
              {/* {status === 'Delivered' ?  <CheckIcon />: status === 'InProgress' ? <Icon icon="eos-icons:hourglass" className="text-white" /> : status === 'Flagged' ? <MyIcon/> : null} */}
              {
                status === 'Delivered' ? (
                  <CheckIcon />
                ) : status === 'InProgress' ? (
                  new Date() >= new Date(ldate) ? (
                    <Icon icon="simple-line-icons:magnifier" className="animate-zoom" />
                  ) : (
            ...
            <p className="text-lg	text-white  uppercase font-normal grid grid-cols-3 gap-x-1 justify-items-end">
              ...
              */}
              {item.reqDot.includes('USD') ? (
                <>
                  <div className="font-extrabold text-xl col-span-2 align-top">{formattedNumber}</div>
                  <Icon icon="heroicons:currency-dollar" className="text-kog-900 w-7 h-7" />
                </>
              ) : (
                <>
                  <div className="font-extrabold text-xl col-span-2 align-top">{item.reqDot}</div>
        ...
        <p className={` ${isSpecialPath ? 'min-h-[120px]' : ''}`}>
          <Link to={item.id} className="text-gray-400 line-clamp-3">{summary}</Link>
        </p>

        <div className="flex justify-between items-center">
          <h6 className="text-kog-900 text-base font-medium mt-2 truncate">
            <Icon icon="ph:user-fill" className="text-white-400 inline border rounded-3xl p-1 border-kog-700 text-3xl mr-2" />

            {scanLink && !scanLink.startsWith('-') ? (
              <Link to={scanLink} target="_blank" className="text-blue-400 truncate">
                {benAdd}
              </Link>
            ) : (
              <span className="text-gray-500 truncate">{benAdd}</span> // Display as plain text if unclickable
            )}
          </h6>
          <p className="border-kog-300 text-kog-white px-3 py-1 rounded-3xl mx-1 p-2 shadow-[0px_0px_10px_2px_rgba(0,0,0,0.2)] transition-all duration-200 ease-in-out bg-gradient-to-t from-kog-100 to-kog-300 inline-flex whitespace-nowrap">
            {category}
          </p>
        </div>
        ...
        {!isSpecialPath && (
          <>
            ...
            <div className="flex justify-between items-center max-w-6 text-sm text-kog-500 my-1">
              ...
              <p className="text-white ">{fdate}</p>
              <p className="text-[#E405F5] text-base font-bold"><Timer startDate={fdate} endDate={ldate} /></p>
              <p className="text-white">{ldate}</p>
