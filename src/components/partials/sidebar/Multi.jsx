...
const LockLink = ({ to, children, item }) => {
  ...
  return (
    <>
      {item.badge ? (
        <span
          className={`text-kog-white dark:text-slate-300 opacity-50 cursor-not-allowed
              ...
              {({ isActive }) => (
                ...
                >
                  <span
                    className={`${
                      isActive
                        ? " bg-slate-900 ring-4 ring-opacity-[15%] ring-slate-300 dark:ring-opacity-20"
                        : ""
                    } h-2 w-2 rounded-full border border-slate-600 dark:border-white inline-block flex-none`}
                  ></span>
                  <span className="flex-1 text-kog-900">{item.multiTitle}</span>
                </span>
              )}
            </LockLink>
          </li>
